...
def duplicate_chunk_block_with_hr(**context):
    ...
    try:
        ...
        logger.info(f"[{task_name}] Starting duplicate_chunk_block_with_hr task with workflowId: {workflow_id}")        

        project_path = context["project_path"]
        project_name = context["project_name"]
        chunk_label = context["chunk_label"]

        logger.info(f"[INFO] Starting duplication for HR chunk from label: {chunk_label}")
        ...
        duplicated_chunk.label = "Chunk_HR"
        doc.chunks.append(duplicated_chunk)
        logger.info(f"[INFO] Duplicated chunk labeled as 'Chunk_HR'")

        removed_count = 0
        for camera in duplicated_chunk.cameras[:]:
            if camera.group and "HL" in camera.group.label:
                duplicated_chunk.remove(camera)
                duplicated_chunk.remove(camera.group)
                removed_count += 1

        logger.info(f"[INFO] Removed {removed_count} 'HL' cameras/groups from duplicated chunk")
